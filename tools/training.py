...
class Model(Container):
        ...
        for i in range(len(self.outputs)):
            ...
            for metric in output_metrics:
                if metric == 'accuracy' or metric == 'acc':
                    ...
                    metric_fn = metrics_module.get(metric) # 多个指标名配置
    ...
    def _fit_loop(self, f, ins, out_labels=[], batch_size=32,
        ...
        index_array = np.arange(nb_train_sample) # [0,n-1]
        ...
        for epoch in range(nb_epoch):
            ...
            if shuffle == 'batch':
                ...
                np.random.shuffle(index_array) # 样本索引打乱

            batches = make_batches(nb_train_sample, batch_size)
            epoch_logs = {}
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                batch_ids = index_array[batch_start:batch_end] # 取batch_size大小的数据,但是是按打乱样本index拿到的
                ...
                try:
                    if type(ins[-1]) is float:
                        ...
                        ins_batch = slice_X(ins, batch_ids) # 拿到batch_ids里面乱序样本index构成最终的batch
                except TypeError:
